...
import { updateMessageWithReaction, updateMessageWithReceipt } from '../Utils/messages'
...
const md5Hash = (data: Buffer | Uint8Array) => createHash('md5').update(data).digest()
...
export default (config: BaileysInMemoryStoreConfig) => {
	...
	const bind = (ev: BaileysEventEmitter) => {
		...
		ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, isLatest, syncType }) => {
			if (syncType === proto.HistorySync.HistorySyncType.ON_DEMAND) {
				return // TODO: handle on-demand history
			}
				...
				if (contacts[update.id!]) {
					...
					const contactHashes = await Promise.all(
						Object.keys(contacts).map(async contactId => {
							...
							return [contactId, md5Hash(Buffer.from(user + 'WA_ADD_NOTIF', 'utf8')).toString('base64').slice(0, 3)]
						})
					)
	...
	return {
		...
		getLabels: () => labels,
		getChatLabels: (chatId: string) => labelAssociations.filter((la: LabelAssociation) => la.chatId === chatId).all(),
		getMessageLabels: (messageId: string) => {
			const associations = labelAssociations
				.filter((la: LabelAssociation): la is MessageLabelAssociation => {
					return la.type === LabelAssociationType.Message && la.messageId === messageId
				})
				.all()
